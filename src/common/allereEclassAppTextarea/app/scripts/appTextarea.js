                ...
                $scope.parseExpr2Html = function () {
                    ...
                    parseResult.forEach(function (item) {
                        ...
                        for (var key in $scope.UNSUPPORT_EXPRESSION_SET) {
                            ...
                            //解决 ios 最新系统，xy渲染换行的问题
                            if(item.match(new RegExp('^'+$scope.UNSUPPORT_EXPRESSION_SET.UNKN.expr),"mg")){
                        ...
                        $wrapper.append($unit);
                        if($unit.attr('mathjax-parser') != undefined){
                            $unit.css('display','block');
                            $unit.css('display','inline-block');
                        }
                    });
            ...
            link: function ($scope, element, $attrs, ctrl) {
                ...
                $scope.$on('keyboard.findNextInput', function (ev, val) {
                    ...
                    if (currentCursor.next().length) {//有下一个元素
                        ...
                        if (nextFracNode.length) {//说明下一个元素是分数
                            if (nextFracNode.children().length != 3)//如果是已经渲染了的分数
                                angular.element(nextFracNode).trigger('click');
                            $timeout(function () {
                                //var clickBeforeFirstNode=true;
                                angular.element(nextFracNode).find('span').eq(2).trigger('click');
                            });
                            return;
                        }
                    ...
                    if ($scope.currentElement.hasClass("fractionInputBoxBottom")) {//如果当前是处于分子的最末尾
                        ...
                        angular.element(fractionInputBoxTop).trigger('click');
                        return;
                    }
                    if ($scope.currentElement.hasClass("fractionInputBoxTop")) {//如果当前是处于分母的最末尾
                        angular.element($scope.currentElement.parent()).trigger('click', true);
                    ...
                    angular.element(nextInput).trigger('click');
                ...
                $scope.$on('keyboard.addContent', function (ev, val) {
                   ...
                   /* let nowInputTime = new Date().getTime();
                    let debounceTime = nowInputTime - lastInputTime;
                    lastInputTime = nowInputTime;
                    if (debounceTime < 250)return; //如果在250毫秒内接受到连续的输入，则忽略*/
                    ...
                    var parser = new ExpressionParser();
                    var data = parser.parse(val); //调用函数处理文字加mathjax表达式的答案,如果有就存为数组
                ...
                function splitUnit(val) {
                    var units = [];
                    for (var key in $scope.EXPRESSION_SET) {
                        if (key == 'WORD_ANSWER'&& val.match($scope.EXPRESSION_SET[key])) {
                            break;
                        }
                        if (key == 'SQUARE') {
                            continue;
                ...
                function addUnitToTextArea(val, forceCompile) {
                    ...
                    if (($currentElement.is('.fractionInputBoxTop') || $currentElement.is('.fractionInputBoxBottom')) && val.match(new RegExp($scope.EXPRESSION_SET.NEWLINE, 'mg'))) {
                        angular.element($currentElement.parent()).trigger('click', true);
                        return; //分数内点击回车就让光标跳过该分数
                    }
                    ...
                    if (($currentElement.is('.fractionInputBoxTop') || $currentElement.is('.fractionInputBoxBottom')) && val.match(new RegExp($scope.EXPRESSION_SET.FRACTION, 'mg'))) {
                        ...
                        if ($currentElement.hasClass("fractionInputBoxTop")) {//如果当前是处于分母的最末尾
                            ...
                            angular.element(fractionInputBoxBottom).trigger('click');
                            return;
                        }
                    ...
                    if (val.startsWith('\\')
                        ...
                        //解决 ios 最新系统，xy渲染换行的问题
                        if(val.match(new RegExp('^'+$scope.UNSUPPORT_EXPRESSION_SET.UNKN.expr),"mg")){
                            $unit.css('display','block');
                            $unit.css('display','inline-block');
                        }
                        if (!isSupportedExpression){
                            $unit.attr('mathjax-parser', '').attr('value', val).attr('ng-bind', 'value');
                            $unit.css('display','block');
                            $unit.css('display','inline-block');
                        }
                        $compile($unit)($scope);
                    ...
                    if (element.hasClass($scope.PULL_INPUT_AREA_CLASS_IDENTITIY)) {
                        ensureSpace();
                    }
                }
                ...
                $scope.$on('keyboard.del', function (ev, val) {
                    ...
                    if ($scope.currentNode) {
                        function doDelete() {
                            /*if(!$scope.currentNode.length)
                             $scope.currentNode=element.find('cursor');*/
                            if ($scope.currentNode.is("cursor"))
                                $scope.currentNode = $scope.currentNode.prev();
                            var $temp = $scope.currentNode;
                            var $prev = $scope.currentNode.prev();
                            var $prevPrev = $scope.currentNode.prev().prev();
                            var $next = $scope.currentNode.next();
                            var $nextNext = $scope.currentNode.next().next();
                            var isComment = false;
                            //在行首点击删除的时候，不能让答语和列式同行
                            if ($temp && $temp.attr('value') === '\\n' && $next.is("cursor") && $nextNext && $nextNext.is('unit[word-answer]'))
                                return;
                            if ($prev.length)
                                setCursorAndCurrentNode($prev.eq(0));
                            else if (val.currentEle && val.currentEle.is('unit[comment]')) {
                            ...
                            if (isComment)
                                $wrapper.find('#' + $next.attr('id')).remove();
                            $scope.textContent.html = $wrapper.html();
                            selfChangeExpr = true;
                            $scope.textContent.expr = $wrapper.text();
                            $temp.remove();
                            ...
                            //屏蔽回删
                            // var currentNode_=($scope.currentNode&&!$scope.currentNode.is("cursor"))?$scope.currentNode:$nextNext;
                            // //当删除到末尾了
                            // if(currentNode_.attr('value')==='\\n'){
                            //     $next.remove();
                            //     currentNode_=$prev;
                            // }
                            ...
                            //解析情况删除掉上一行
                            if ($scope.currentNode && $scope.currentNode.attr('value') === '\\n') {
                                $next.remove();
                                $scope.currentNode = $prev;
                            }
                            //在渲染内容时，需要判断下当前输入是否为“=”,且为解方程的情况
                            if (alineByEqual.currentQIsEquation(element)) {
                                $timeout(function () {
                ...
                function elementClick() {
                    angular.element(element).on('click', function (ev) {
                        ev.stopPropagation();
                        //如果当前点击的是分数且该分数是在选择输入框内，就不处理点击事件。
                        if ($(element).is('unit[frac]') && $(element).parent().hasClass('select-input-area')) return;
                        if (($(element).is('fractionInputBoxTop') || $(element).is('fractionInputBoxBottom')) && $(element).parent().parent().hasClass('select-input-area')) return;

                        var ionSlidePage = $(element).parents('ion-slide');//如果不是当前ionSlide就返回
                        var clickIsBefore = true;
                        ...
                        if ($target[0] && $target[0].nodeName.toUpperCase() == "UNIT") {
                            //if(ev.clientX)
                            clickIsBefore = (ev.clientX - $target.offset().left) <= $scope.CLICK_INSENSITIVITY_VALUE;
                ...
                $scope.$root.bodyClickFn = $scope.$root.bodyClickFn ? $scope.$root.bodyClickFn : function (ev) {
                    ...
                    if (!(ev.target.nodeName.toUpperCase() == 'APP-TEXTAREA')
                        && !$target.parents('app-textarea, vertical, .appTextarea').length
                        && !$target.hasClass('appTextarea')
                        // && (!$target.parents('.simplify-textarea').length && !$target.is('.simplify-textarea'))
