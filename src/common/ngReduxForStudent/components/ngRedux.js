...
export default function ngReduxProvider() {
    ...
    this.$get = ($injector) => {
        ...
        store = _initialState
            ? applyMiddleware(...resolvedMiddleware)(finalCreateStore)(p_reducer, _initialState)
            : applyMiddleware(...resolvedMiddleware)(finalCreateStore)(p_reducer);
        let mergeReducer = function (reducer, defaultStates, reducerKey) {
            Object.assign(_reducerObj, reducer);
            p_reducer.reducer = combineReducers(_reducerObj);
            return new Promise(resolve => {
                // store.dispatch({type:"@@redux/INIT"})
                store.update(reducer,defaultStates, function () {
                    resolve();
                })
            });
        };
        return assign({}, store, {connect: Connector(store)}, {mergeReducer});
    };
