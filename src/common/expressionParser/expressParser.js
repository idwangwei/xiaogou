...
class ExpressionParser {
    constructor() {
        ...
        this.idFlag = 0;
        this.combineExpSet = ['\\\\var{.*}', '\\\\frac{.*}{.*}', '\\\\wordAnswer{.*}', '\\\\unit{.*}{.*}{.*}', '\\\\comment{.*}', '\\\\dot{.+?}'];
        this.R_BRACKETS = ['}'];
        this.replaceMap = {};
    ...
    getCombineExpSet() {
        ...
        for (var key in expressionSet) {
            if (key == 'SQUARE') {
                list.push(new RegExp('^' + expressionSet[key]));
            } else {
                var word = this.getRegExpWord(expressionSet[key]);
                if (word == '') {
                    list.push(new RegExp('^' + '    '));//空格做特殊处理
                } else {
                    list.push(new RegExp('^\\\\' + word));
                }
            }
        }
        return list;
    ...
    peekStr() {
        ...
        for (var i = 0; i < this.expSet.length; i++) {
            var find = false;
            this.parseString = this.parseString.replace(this.expSet[i], function (match) {
                find = true;
                matchStr = match;
                return '';
            });
    ...
    restoreFromReplaceMap() {
        ...
        for (key in this.replaceMap) {
            val = this.replaceMap[key];
            for (key2 in copiedMap) {
                val = val.replace(key2, copiedMap[key2]);
            }
            this.replaceMap[key] = val;
        }
        let me = this;
        this.operationStack.forEach(function (item, idx) {
            for (key in me.replaceMap) {
                if (key == item) {
