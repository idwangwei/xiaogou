...
var appid = 'D8060C618C0B4E749C6DD4F04FD00229';
var vn = '学生端';
var vc = '1.0.1';
var TDRequestUrl = 'https://h5.udrig.com/app/v1';

function pageLogout(e) {
		localStorage.setItem('test____', '1111');
	...
	function (e, t, n) {
			...
			var e = function (e) {
					...
					if (e == "object") {
							...
							typeof e == "function" && (this.hasher = e)
					}
			};
			return e.prototype = {
					get: function () {
...
(function (e) {
		e.TDBASE = e.TDBASE || {},
			...
			TDBASE.getArgs = function () {
					var t = new Object, n = e.location.search.substring(1), r = n.split("&");
			...
			TDBASE.addGenre = function (e, t, n) {
					if (!!localData.get("sessionMsg")) {
							n && n(e);
							var r = JSON.parse(localData.get("sessionMsg")), i = r.msg[r.msg.length - 1].data[t];
							if (t === "events") {
									if (i.length != 0) {
											var s = !1;
											for (var o = 0; o < i.length; o++)if (TDBASE.equal(i[o], e)) {
			...
			TDBASE.addMsg = function (e) {
					if (!localData.get("sessionMsg"))localData.add("sessionMsg", JSON.stringify({msg: [e]})); else {
...
var localData = TalkingData.localStorage,
	...
	DTMgr = {
			...
			set: function (loginName) {
					try {
							this.setDeviceId(loginName), this.setSession(), this.setSessionTime(), this.setInitTime(), this.setPartner(), this.setResolution(), this.addlastSession(), this.getAjax()
					} catch (e) {
							console.log(e)
					}
			},
			setDeviceId: function (loginName) {
					if (localData.get("deviceId"))this.deviceId = localData.get("deviceId"); else {
							var e = new TalkingData.Fingerprint, t = new TalkingData.Fingerprint({screen_resolution: !0}), n = e.get() + "" + t.get();
							parseInt(n) == NaN ? this.deviceId = n : this.deviceId = parseInt(n).toString(16);
							this.deviceId = this.deviceId.replace(/\.|\+|\(|\)/g, "");
					}
					if (loginName) {
							var s1 = parseInt(loginName.substr(0, 11)).toString(16);
							var str = loginName.substr(11);
							var s2 = 0;
							for (let i = 0, len = str.length; i < len; i++) {
									s2 += str.charCodeAt(i);
							}
							var phoneNum = s1 + parseInt(s2).toString(16);
							
							var reg = new RegExp("^.{" + phoneNum.length + "}");
							this.deviceId = this.deviceId.replace(reg, phoneNum);
							DTGlobal.appProfile.versionName = loginName.match(/s/i) ? '学生端' : loginName.match(/t/i) ? '教师端' : '家长端';
					}
			...
			setSession: function () {
					...
					sessionData.add("sessionId", e);
			},
			setSessionTime: function () {
					var e = (new Date).getTime();
					sessionData.add("SessionStart", e);
					AppSession.data.start = parseInt(e)
			},
			setInitTime: function () {
					localData.get("initTime") ?
						DTGlobal.appProfile.initTime = parseInt(localData.get("initTime"))
						: (DTGlobal.appProfile.initTime = WebappStart, localData.add("initTime", WebappStart))
			},
			setPartner: function () {
					var e = sessionData.get("td_channel"), t = TDBASE.getArgs().td_channelid;
					if (e) {
			...
			addlastSession: function () {
					...
					if (!e)TDBASE.addSessionStart(0, 1); else {
							...
							if (t.id != AppSession.data["id"]) {
									...
									TDBASE.addMsg(n);
									this.timedif = parseInt((AppSession.data.start - localData.get("leavetime")) / 1e3);
			...
			getAjax: function () {
					...
					var n = JSON.parse(localData.get("sessionMsg")), r = TDBASE.getCommon(n), i = localStorage["__TD_td-init-event"];
					if (i) {
			...
			clearData: function () {
					localData.delLocalStorage('appkey');
					localData.delLocalStorage('deviceId');
					localData.delLocalStorage('lastSession');
					localData.delLocalStorage('leaveSession');
					localData.delLocalStorage('leavetime');
					localData.delLocalStorage('sessionMsg');
					sessionData.remove("SessionStart");
					sessionData.remove("sessionId");
			},
			send: function (random,loginName) {
					var num = Math.random().toFixed(2);
					var localRandomDate = localData.get("randomDate");
					var canRandom = !localRandomDate || new Date(+localRandomDate).getDate()<new Date().getDate();
					//增加一次数据(random概率)
					if (num < random && canRandom) {
							var generateName = loginName.length > 12 ? loginName.substr(0, 12) : loginName + "1";
							this.set(generateName);
							setTimeout((function () {
									this.clearData();
									this.set(loginName);
							}).bind(this), 2000);
							localData.add('randomDate',new Date().getTime())
					}else {
							this.set(loginName)
					}
			}
				...
				if (arguments.length > 0)try {
						...
						if (TDXHR) {
								TDBASE.addGenre(r, "events", function (e) {
										...
										t = JSON.parse(t);
										for (var r = 0; r < t.length; r++)if (TDBASE.equal(t[r], e)) {
