(angular => {
    ...
    angular.module('oc.lazyLoad').config(function ($provide) {
        $provide.decorator('$ocLazyLoad', function ($delegate, $q, $rootScope) {
            /**
            ...
            $delegate.load = function (originalModule, originalParams = {}) {
                ...
                $delegate.filesLoader(config, localParams).then(() => {
                    /*let mdToLoad = $delegate.getModulesToLoad();
                    let thisModulesToLoad=mdToLoad.slice();
                    mdToLoad.length=0;*/
                    if ($rootScope[`${localParams.name}-storeReady`]) {
                        $delegate.injectThis(null, localParams,false).then(res => {
                            deferred.resolve(res);
                        }, err => {
                            console.log(err)
                        ...
                        let offFn = $rootScope.$on(`${localParams.name}-storeReady`, () => {
                            $rootScope[`${localParams.name}-storeReady`] = true;
                            $delegate.injectThis(null, localParams,false).then(res => {
                                deferred.resolve(res);
                            }, err => {
                                console.log(err)
