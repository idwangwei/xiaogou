    ...
    ocLazyLoad.provider('$ocLazyLoad', function ($controllerProvider, $provide, $compileProvider, $filterProvider, $injector, $animateProvider) {
        ...
        this._init = function _init(element) {
            ...
            if (modulesToLoad.length === 0 && !((window.jasmine || window.mocha) && angular.isDefined(angular.mock))) {
                console.error('No module found during bootstrap, unable to init oc-lazy-load. You should always use the ng-app directive or angular.boostrap when you use oc-lazy-load.');
            }
        ...
        this.$get = function ($log, $rootElement, $rootScope, $cacheFactory, $q) {
            ...
            return {
                ...
                /*
                 inject Only this  2018-2-26
                 merge and store has problem
                 */

                injectThis: function (moduleName, localParams = {}, real = false) {
                    var self = this,
                        deferred = $q.defer();
                    if (angular.isDefined(moduleName) && moduleName !== null) {
                        if (angular.isArray(moduleName)) {
                    ...
                    /*计算出当前的队列 start*/
                    let modulesThis = [], spliceNum = [];
                    for (var i = 0; i < modulesToLoad.length; i++) {
                        if (modulesToLoad[i] === localParams.name || modulesToLoad[i].indexOf(localParams.name + '.') != -1) {
                            modulesThis.push(modulesToLoad[i])
                            spliceNum.push(i - spliceNum.length);
                        }
                    }
                    for (var i = 0; i < spliceNum.length; i++) {
                        modulesToLoad.splice(spliceNum[i], 1);
                    }
                    /*计算出当前的队列 end*/
                    if (modulesThis.length > 0) {
                        var res = modulesThis.slice(); // clean copy
                        var loadNext = function loadNext(moduleName) {
                            // console.log("inject "+moduleName+" start=========")
                            moduleCache.push(moduleName);
                            modulePromises[moduleName] = deferred.promise;
                            self._loadDependencies(moduleName, localParams).then(function success() {
                                try {
                                ...
                                if (modulesThis.length > 0) {
                                    loadNext(modulesThis.shift()); // load the next in list
                                } else {
                                    deferred.resolve(res); // everything has been loaded, resolve
                                }
                            }, function error(err) {
