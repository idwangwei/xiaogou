        ...
        controller: ["$scope", "$element", "$attrs", "EXPRESSION_SET", "commonConstants", function ($scope, $element, $attrs, EXPRESSION_SET, commonConstants) {
            ...
            var watchFn = $scope.$watch('value', function () {
                ...
                if (flag) {
                    ...
                    //处理mathjax 渲染value="a\\ltb"时的错误（a \\ltb ），正确为a<b
                    value = value.replace(new RegExp(`(${specialHandle.join('|')})([a-zA-Z])`,'g'),(match,$1,$2)=>{
                        return $1+" "+$2;
                    });
                ...
                //由于错题记录在做题路径里面，此时框子内有分数，就不会执行批改了，下面两行是补丁
                var smallQEle = $($element).parents('[compile-html]');

                var stateName=smallQEle.attr('state-name')?smallQEle.attr('state-name'):$state.current.name;
                //如果没有启用延迟编辑选项，则马上进行编译
                if (((stateName == STATE_FOR_EDIT && $element.parent().attr('lazy-compile') != 'true') || stateName != STATE_FOR_EDIT) && ($attrs.lazyCompile == "false" || !$attrs.lazyCompile)) {
                    ...
                    if (stateName == STATE_FOR_EDIT && $element.parents('[fraccount]').length) {//说明该输入框有分数
                        ...
                        if ((faccInt + "") === "0") {//这里表示拿到做题内容，一个个渲染，此时已渲染到最后一个分数了
                            renderMathJaxOne($element);
                            //$scope.$emit('question.slide.change');
                        }
