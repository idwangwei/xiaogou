...
module.directive('draft',['$compile',function($compile,$templateCache){
    return {
            restrict: 'E',
//            template: canvasUrl,
...
//                $scope.footerHight = $('ion-view .bar-footer').eq(0).css('height').match(/\d+/g);
            ...
            link: function postLink($scope, elm, attr) {
                ...
                var canvasHtml =`<div class="container draft_container" style="width: 100%">
                    <div class="pwCanvasPaint" style="position:fixed; left: 0; width: 100%"></div>
                   <img ng-src="{{$root.loadImg('do_question/close_draft.png')}}" alt=""  " ng-show='doQCtrl.showDraft'
                        style="width: 30px;height: 30px;position: absolute;z-index: 1000;top: 10px;right: 10px;" ng-click="hideTheDraft()">  
                </div>`;
                var res = $($compile(canvasHtml)($scope));
                $("body").append(res);
                ...
                $scope.hideTheDraft=function(){
                    $scope.$broadcast('draft.hide');
                };
                window.ctx = ctx;
