...
class QCorrectAnsCtrl {
    ...
    getQAns(savantAnsFlag, ev) {
        ...
        this.workStatisticsServices.getQReferAnswer(param, savantAnsFlag).then(function (data) {
            if (data) {
                // let tempData={
     ...
     * @param ev
     */
    getXBAns(savantAnsFlag) {
        var me = this;
        console.log('type---------',this.questionTypeKey);
        if (this.questionTypeKey === 'notSubjective' && savantAnsFlag) return;
        var param = {
            questionId: this.$scope.smallQ.id
        };
        this.workStatisticsServices.getQReferAnswer(param, savantAnsFlag).then(function (data) {
            if (data) {
                ...
                let noReferAns = false;
                //判断答案是否是空对象 和 是不是学霸解答
                if (Object.keys(data.referAnswers || {}).length == 0 && savantAnsFlag) {
                    noReferAns = true;
                    // return;
                }
                if (data.referAnswers && data.referAnswers.vertical) {
                    me.isVertical = true;
                    me.parsedInputList = me.clearVerticalQAns(angular.copy(me.$scope.smallQ.inputList));
                }
                if(!savantAnsFlag) {
                ...
                if(savantAnsFlag||me.questionTypeKey === 'notSubjective'||noReferAns){
                    me.showCorrectFlag = false;
                    me.showDoc = false;
                    me.doc = null;
                    me.parsedInputList = null;
