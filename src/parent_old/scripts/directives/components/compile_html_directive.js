...
directives.directive('compileHtml', ['$compile', '$log', '$filter','commonConstants','verticalService', function ($compile, $log, $filter, commonConstants,verticalService) {
    return {
        restrict: "A",
        scope: {
        ...
        link: function ($scope, ele, attrs, ctrl) {
            ...
            if ($scope.showType == "correct") {
                ...
                if ($wrapper.find('.set-list').length) {
                    ...
                    $wrapper.find('.set-add-icon').removeAttr('disabled', false).css('display', 'inline-block');
                    $wrapper.find('.set-remove-icon').removeAttr('disabled', false).css('display', 'inline-block');
                }
            }
            ...
            if ($scope.referAnswers) {
                _.each($scope.referAnswers, function (inputBoxStuAns, index) {//由于后台给的referAnswers是对象数组
            ...
            if ($scope.currentQInput && $scope.currentQInput.length > 0) {
                _.each($scope.currentQInput, function (spInfo) {
                    _.each(spInfo.spList, function (inputBoxInfo, index) {
