...
@View('sprint_goods_weixin_pay', {
    url: '/sprint_goods_weixin_pay',
    template: '<sprint-goods-weixin-pay></sprint-goods-weixin-pay>'
})

@Inject('$scope', '$state', '$rootScope', '$stateParams', '$ngRedux', 'finalSprintService','commonService','$ionicPopup')
class sprintGoodsWeixinPayCtrl {
    ...
    orderType = this.finalSprintService.orderType;
    ...
    createOrderWxPayApp() {
        ...
        this.finalSprintService.createOrder(this.productId, this.payAppCode, "app").then(data=> {
            if (data.code == 200) {
                this.sendWxPayRequest();
                return;
            }
    ...
    createOrderWxPayScan() {
        ...
        this.finalSprintService.createOrder(this.productId, this.payAppCode, "scan").then(data=> {
            if (data.code == 200) {
                this.showQrCode(); //显示二维码
                this.iscreateScanOrder = true;
                return;
