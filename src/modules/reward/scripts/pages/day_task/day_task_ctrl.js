...
@View('reward_day_task', {
    ...
    template: '<day-task></day-task>'
})

@Inject('$scope', '$state', '$rootScope', '$stateParams', '$ngRedux', '$ionicHistory', 'rewardSystemService', '$ionicLoading', 'olympicMathService')
class dayTaskCtrl {
    ...
    mapActionToThis() {
        let oms = this.olympicMathService;
        return {
            changeUrlFromForStore: oms.changeUrlFromForStore.bind(oms)
        }
    }
