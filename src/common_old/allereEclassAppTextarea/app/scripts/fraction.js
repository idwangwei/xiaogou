...
import  $ from 'jquery';
module.directive('frac', function ($compile, $q,$state, $window, $timeout, UNSUPPORT_EXPRESSION_SET) {
    "ngInject";
    return {
        restrict: "A",
        scope: {
        ...
        link: function ($scope, element, attrs) {
            ...
            function lazyCompileFrac(){
                if ($state.current.name == STATE_FOR_EDIT&&!keyBoardIsShow()) { //在学生端做题的页面并且没有键盘情况
            ...
            function addFractionEditUI() {
                ...
                if ($state.current.name == STATE_FOR_EDIT&&keyBoardIsShow()) { //在学生端做题的页面自动进入编辑模式
                    var ionSlidePage=$(element).parents('ion-slide');//如果不是当前ionSlide就返回
                    if(ionSlidePage.length!=0&&ionSlidePage.attr('data-index')!=ionSlidePage.attr('slideboxcurrentindex'))return;
                    $timeout(function () {
                        ...
                        if ((!topExpr || !bottomExpr)&&flag!='true'){
                            ...
                            $(element).find('span').eq(2).trigger('click');
                        }
                    });
		    return;
                }
                ...
                if ($state.current.name == STATE_FOR_EDIT) { //在学生端做题的页面自动进入编辑模式
                   ...
                   var withoutChildren=$(element).find('.fractionInputBoxTop').children('unit').length==0&&$(element).find('.fractionInputBoxBottom').children('unit').length==0;
                    ...
                    if(fracNull&&withoutChildren){
                        ...
                        var parent=$(element).parent();
                        parent.scope().$emit('keyboard.del', {ele: parent,currentEle:$(element)});
            ...
            element.on('click', function (ev) {//点击分数时显示分数编辑框
                ...
                if (isMathjaxState)
                    ev.stopPropagation();
                if ($state.current.name != STATE_FOR_EDIT||$(element).parent().hasClass('comment'))
                    return;
                $scope.$apply(()=>{
                    addFractionEditUI();
                });
