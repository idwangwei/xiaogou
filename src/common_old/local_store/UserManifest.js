...
class UserManifest {
    ...
    addUserToUserList(user) {
        ...
        /*this.userList.forEach((item, index)=> {
         ...
         if ((isNew&& item.loginName.toLowerCase() === user.loginName.toLowerCase())||(!isNew&& item === user.loginName)) findIndex = index;*!/

         });*/
        try {
            this.userList.find((item, index) => {
                if (item.loginName.toLowerCase() === user.loginName.toLowerCase()) {
                    findIndex = index;
                }
            })
        } catch (err) {
   ...
   /* /!**
    ...
    getPasswordCode(pw) {
        ...
        let randomCode = _.sample(this.randomCodeItems, 6);
