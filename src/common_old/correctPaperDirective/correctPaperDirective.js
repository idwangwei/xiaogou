...
correctPaperDirective.directive('correctPaper', function ($compile, $log, $timeout) {//根据IMG_SERVER配置的地址动态加载图片
    'ngInject';
    return {
        restrict: 'EA',
        scope: true,
        ...
        controller: ['$scope', 'uuid4', 'finalData', function ($scope, uuid4, finalData) {
            ...
            $scope.getCorrectHtmlStr = function (scorePointId) {
                ...
                return {areaImgHtml: areaImgHtml, yesImgHtml: yesImgHtml, yesImg2Html:yesImg2Html,
            ...
            $scope.specialQNomalExpHandle = function (spInfo, arr) {
                ...
                _.each(arr, function (data) {
                    ...
                    for (var i = 0; i < unitEleArray.length; i++) {
                        if (startNodeCount <= i && i <= totalCount - lastNodeCount) {//表示当前是目标截取的元素之内的
