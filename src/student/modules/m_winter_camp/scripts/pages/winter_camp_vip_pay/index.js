...
@View('winter_camp_vip_pay', {
    url: '/winter_camp_vip_pay',
...
class WinterCampVipPayCtrl {
    $stateParams;
    winterCampService;
    commonService;
    @select(state => state.winter_camp_vip.select_goods) selectGoods;
    @select(state => state.winter_camp_vip.created_order_info.app) wxAppOrderInfo;
    @select(state => state.winter_camp_vip.created_order_info.scan) wxScanOrderInfo;
    orderType = this.winterCampService.orderType;
    ...
    back() {
        this.go("winter_camp_vip_select");
    ...
    createOrderWxPayApp() {
        ...
        this.winterCampService.createOrder(this.selectGoods.id, this.payAppCode, "app").then(data=> {
            if (data.code == 200) {
                this.sendWxPayRequest();
                return;
            }
    ...
    createOrderWxPayScan() {
        ...
        this.winterCampService.createOrder(this.selectGoods.id, this.payAppCode, "scan").then(data=> {
    ...
    isCreateNewOrder(type) {
        ...
        if (!orderInfo || (orderInfo && orderInfo.productId != this.selectGoods.id) || (orderInfo && orderInfo.status === 0 && orderInfo.productId == this.selectGoods.id)) {
            return true;
        }
     ...
     */
    gotoPayResult() {
        this.go("winter_camp_vip_result",{orderType:this.currentOrderType});
    }
    ...
    gotoProtocol($event) {
        ...
        this.go("winter_camp_vip_protocol");
    }
}

export default WinterCampVipPayCtrl;