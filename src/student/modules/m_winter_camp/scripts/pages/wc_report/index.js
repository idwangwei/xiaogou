...
@View('wc_report', {
    url: '/wc_report/:urlFrom/:pointIndex/:pointName/:backWorkReportUrl',
...
class DiagnoseReportCtrl {
    ...
    back(){
        if (this.backWorkReportUrl) {
            this.go('wc_diagnose_do_question', 'forward', {
                'pointName': this.getStateService().params.pointName,
                'backWorkReportUrl': this.backWorkReportUrl
            });
        } else {
            this.go('wc_diagnose_do_question', 'forward', {'pointName': this.getStateService().params.pointName});
        }
    }

    gotoDiagnoseDoQues(){
        if(this.backWorkReportUrl){
            this.go('wc_diagnose_do_question','farword',{'pointName':this.getStateService().params.pointName,'backWorkReportUrl':this.backWorkReportUrl});
        }else{
            this.go('wc_diagnose_do_question','farword',{'pointName':this.getStateService().params.pointName});
        }
