...
services.service('localstorageDbUtil', ['$log', '$rootScope', 'commonService', 'finalData', 'dateUtil',
    function ($log, $rootScope, commonService, finalData, dateUtil) {
        ...
        if ($rootScope.user && $rootScope.user.userName && $rootScope.user.userName != "") {
            USER_NAME = $rootScope.user.userName + "同学，智算365欢迎您!";
        } else {
        ...
        this.save = function (dbName, data, key) {
            ...
            if (data instanceof  Array) {
                ...
                _each(data, function (item) {
                    var index = -1;
                    _each(dbCopy, function (record, i) {
        ...
        var handleMsgInit = function (dbName) {
            if (dbName == finalData.DB_CONTACTS_S || dbName == finalData.DB_MSG_S) {
                var senderDb = [];
                var msgDb = [];
                ...
                var senderInfo = {
                    ...
                    senderName: "智算365",
                    senderRole: "S",
                    revId:$rootScope.user.userId   //TODO:临时这样处理可能会有bug
                };
                var msgInfo = {
                    msgId: "1",
                    senderId: "1",
                ...
                msgDb.push(msgInfo);
                $log.log(msgDb);
                commonService.setLocalStorage(finalData.DB_CONTACTS_S, senderDb);
                commonService.setLocalStorage(finalData.DB_MSG_S, msgDb);
            }
        }
