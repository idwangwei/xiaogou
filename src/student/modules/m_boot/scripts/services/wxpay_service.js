...
let GAME_GOODS_PAY = {UPDATE_GAME_VIP: 'UPDATE_GAME_VIP'};
...
class WxPayService extends BaseService {
    ...
    constructor() {
        ...
        this.cancelTrainingCampRequestList = []; //学霸训练营的请求
    }
    ...
    createXlyVipOrder(productId, thirdPay, payType) {
        let me = this;
        return (dispatch)=> {
            var defer = me.$q.defer();
            dispatch({type: WEIXIN_PAY.FETCH_WEI_XIN_PAY_CREATE_ORDER_START});
            ...
            me.commonService.commonPost(me.serverInterface.XLY_BUYING_ORDER, {
                ...
                if(data.vips){
                    data.vips.forEach((item)=>{
                        if(item.hasOwnProperty('mlcg')){mlcg = item.mlcg;}
                        if(item.hasOwnProperty('diagnose')){diagnose = item.diagnose;}
                        if(item.hasOwnProperty('mathematicalOlympiad')){mathematicalOlympiad = item.mathematicalOlympiad;}
                    });
                }
    ...
    getXlyVipAllGoodsInfo(){
        let defer = this.$q.defer();
        this.commonService.commonPost(this.serverInterface.FETCH_XLY_VIP_GOODS_INFO,{category:'XN-SVIP'}).then(data=> {
            if(data.code === 200){
                defer.resolve(data);
            }else {
                defer.resolve(false);
    ...
    fetchIncreaseScoreGoodsMenus(selectedGrade){
        let me = this;
        return (dispatch, getState)=> {
            let vipKnowledgeGradeArr = [],
                vips = getState().profile_user_auth.user.vips;
            ...
            postInfo.requestPromise.then((data)=> {
                ...
                if (data.code === 200) {
                    ...
                    data.goods.forEach((item)=> {
                        item.desc = JSON.parse(item.desc);
                        item.selected = false;
                        item.hasBought = vipKnowledgeGradeArr.find((v)=>{return v == item.desc.grade}) !== undefined;
                    ...
                    pre.sort((item1,item2)=>{return item1.desc.grade-item2.desc.grade});
                    list.splice(0, 0, ...pre);
                    dispatch({
...
WxPayService.$inject = ['$http', '$q', '$rootScope', 'serverInterface', 'commonService', "$ngRedux",'profileService'];
