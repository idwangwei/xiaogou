...
class WorkDetailCtrl{
    ...
    onUpdate() {
        this.paperFetched = false;//很关键 不然有时候会出问题
        this.retFlag = false;
        // this.ensurePaperFetched();
    }
    ...
    showTip() {
        ...
        // if (!this.alertTipInfo.workDetail) {
        //     var androidOrIosStr = `<p style="font-size:16px" >极少的情况下，若APP出问题，可先试试清理内存，再重启APP：</p> `;
        //     var computerStr = `<p style="font-size:16px" >极少的情况下，若APP出问题，可先试试关掉APP窗口，再重启APP。</p> `;
        //     var sysType = $rootScope.platform.type = $rootScope.platform.IS_ANDROID ? 1 :
        ...
        //             break;
        //     }
        //     this.getScope().checkNoTip = this.checkNoTip.bind(this);
        ...
        //         scope: this.getScope()
        //     }).then(() => {
        //         if (this.noTip.status)
        //             this.commonService.setLocalStorage(this.getState().profile_user_auth.user.userId + "workDetail", true)
        //     });
        // }
    }
    ...
    back() {
        ...
        if(this.selectedWork.publishType == this.WORK_TYPE.AREA_EVALUATION){
            fromUrl = this.finalData.URL_FROM.AREA_EVALUATION;
        }
