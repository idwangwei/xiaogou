...
import {PromiseFn} from "ngDecoratorForStudent/ng-decorator";
import {Inject, View, Directive, select} from '../../module';

@View('select_question', {
    url: '/select_question/:paperId/:paperInstanceId/:redoFlag/:urlFrom/:questionIndex/:urlFromMark',
    template: require('./select_question.html'),
    styles: require('./select_question.less'),
    inject: ["$scope",
        '$rootScope',
        "$log",
        "$state",
...
class SelectQuestionCtrl{
    ...
    onAfterEnterView() {
        ...
        this.isAreaEvaluation = this.selectedWork.publishType == this.finalData.WORK_TYPE.AREA_EVALUATION;
    ...
    timeEndCallBack(retMsg) {
        ...
        this.commonService.showAlert(data.title, data.msg).then((res) => {
            if (res) {//点击确认
                retMsg ? this.go("home.work_list", 'back') : me.submitAnsAndPaper(me.submitCallback.bind(me), true,undefined,undefined,this.getStateService().params.redoFlag);//改错后返回试卷列表
            }
        });
    }
    ...
    mapStateToThis(state) {
        ...
        return {
            ...
            paperTime: (selectedWork.publishType == 13||selectedWork.publishType==16) ? finalAccessPaperTime:oralPaperTime,
        }
    }
    ...
    goToCheck(bigQ, smallQ) {
        ...
        // if(this.title.indexOf("口算")){
        //     this.go("oral_calculation_do_question", "forward", {
    ...
    back() {
        ...
        //期末测评
        if ((this.isFinalAccess||this.isAreaEvaluation) && this.paperTime && this.paperTime.startTime) {
    ...
    submitCallback(msg) {
        ...
        this.$timeout(() => {  //不要删除这个定时器，不然做题时间会有问题
            if (this.selectedWork.publishType == this.finalData.WORK_TYPE.ORAL_WORK_KEYBOARD
                ...
                || this.selectedWork.publishType == this.finalData.WORK_TYPE.AREA_EVALUATION
    ...
    submitPaperHandle() {
        ...
        //期末测评作业提交提示
        if (this.isFinalAccess||this.isAreaEvaluation) {
            if(this.getStateService().params.redoFlag === 'true'){
                data.msg = '你确定要提交吗？'
            }else{
                data.msg = '试卷成绩以首次提交为准，请仔细作答。是否提交？'
                /*新增5分钟内不能提交的逻辑*/
                let time=new Date().getTime();
                if(this.paperTime){
                    time=this.paperTime.startTime;
                }
                let diff = Math.floor((new Date().getTime() - time) / 1000)/60;
                if(diff<5&&this.selectedWork.limitTime>=5){
                    data.msg="答题5分钟后才能提交试卷。"
                    this.commonService.showAlert(data.title, data.msg);
                    return;
                }
            }
            /*end*/
        ...
        this.commonService.showConfirm(data.title, data.msg).then((res) => {
            if (res) {//点击确认
                ...
                if ((this.selectedWork.publishType == this.finalData.WORK_TYPE.ORAL_WORK_KEYBOARD
                        || this.selectedWork.publishType == this.finalData.WORK_TYPE.ORAL_WORK
                        || this.selectedWork.publishType == this.finalData.WORK_TYPE.AREA_EVALUATION
                        || this.selectedWork.publishType == this.finalData.WORK_TYPE.FINAL_ACCESS) && this.paperTime) {
                ...
                me.submitAnsAndPaper(me.submitCallback.bind(me), false, undefined, spentTime ? spentTime * 1000 : undefined,this.getStateService().params.redoFlag);
            }
        });
    };
