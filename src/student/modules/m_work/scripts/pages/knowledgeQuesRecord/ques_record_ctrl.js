...
@View('ques_record', {
    url: '/ques_record/:pointIndex/:urlFrom/:formText',
    template: require('./ques_record.html'),
    styles: require('./ques_record.less'),
    inject:['$scope', '$state', '$stateParams', 'commonService', 'profileService', '$ngRedux', '$ionicPopup', '$ionicTabsDelegate'
    , '$timeout', '$ionicSideMenuDelegate', 'subHeaderService', 'dateUtil', 'diagnoseService', 'tabItemService']
})
class quesRecordCtrl {
    // @select(state=>state.chapter_select_point) chapterSelectPoint;
    ...
    // @select(state=>state.ques_record) workReports; TODO
    ...
    initData() {
        ...
        this.isFinalSprintCheckFlag=this.getStateService().params.urlFrom=="final_sprint_check"
    }
    ...
    back() {
        ...
        if (this.getStateService().params.urlFrom === 'final_sprint_check') {
            this.go('final_sprint_check');
            return;
        }
        if (this.getStateService().params.urlFrom === 'final_sprint_train') {
            this.go('final_sprint_train');
            return;
        }
        this.go('work_report', 'back');
