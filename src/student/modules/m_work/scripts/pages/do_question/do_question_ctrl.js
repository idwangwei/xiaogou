...
@View('do_question', {
    ...
    template: require('./do_quesiton.html'),
    controllerAs:'doCtrl',
    styles: require('./do_question.less'),
    inject:["$log",
        "$ionicScrollDelegate",
        "$state",
        "$scope",
...
class DoQuestionCtrl{
    ...
    initFinalSprint() {
        ...
        if (param.urlFrom == "finalSprint" && param.isFirst == 'yes') {
    ...
    onAfterEnterView() {
        ...
        /* let $closeBtn = $(".q-error-record-btn-close");
         ...
         let $openBtn = $(".q-error-record-btn-open");
         $openBtn[0].style.removeProperty('left');
         $openBtn.removeClass('ng-enter').addClass('ng-leave');
         if ($closeBtn.length) {
         $($closeBtn)[0].style.top = $openBtn[0].style.top;
         $($closeBtn).addClass('active');
         }
         });*/
        this.startTimeFlag = !!this.paperTime;
        if (this.redoFlag === "false")//试卷没有提交则显示广告
    ...
    back(flag) {
        ...
        if (this.getStateService().params.urlFrom === "oral_calculation_select_question") {
            this.go("oral_calculation_select_question", 'back', param);
        } else
            this.go("select_question", 'back', param);
    };
     ...
     */
    findNextNoCheckedQ() {
        let bigQLength = this.bigQList.length, findBigQIndex = -1, findQIndex = -1,
            statusMapForQuestions = this.statusMapForQuestions;
        ...
        if (this.getStateService().params.urlFrom == "finalSprint" || this.getStateService().params.urlFromMark == "finalSprint") {
            param.urlFrom = "finalSprint";
        }
        this.go("select_question", 'back', param);
