...
@View('live_select_question', {
    url: '/live_select_question/:paperId/:paperInstanceId/:redoFlag/:urlFrom/:questionIndex/:urlFromMark',
...
class SelectQuestionCtrl {
    ...
    mapStateToThis(state) {
        ...
        return {
            ...
            selectWork: state.wl_selected_work,
            selectCourse:state.select_course,
            userInfo:state.profile_user_auth.user
        }
    }
    ...
    goToCheck(bigQ, smallQ) {
        ...
        if(cal<=0&&this.selectWork.liveToPay==1){
            this.$ionicPopup.alert({
                title: '信息提示',
                template: '<p>考试时间已过，请提交试卷</p>',
                okText: '确定'
            })
            return
        }
        ...
        this.go("live_do_question", "forward", param);
    };

    goUrlHandle() {
        /*新增期末冲刺逻辑*/
        let params = this.getStateService().params;
        if (this.selectWork.liveToPay) {
            this.go("live_course_fee_paper");
            return;
        }
        if (params.urlFrom == "live") {
            this.go("home.live_home")
            return;
        }
        if (params.urlFrom == "detail" && params.urlFromMark == "live") {
            this.go("live_work_detail", 'back', {urlFrom: 'live'});
            return;
        }
        this.go("live_work_detail", 'back');
    }
