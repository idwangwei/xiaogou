...
@View('live_course_pay', {
    url: '/live_course_pay/:fee',
...
class liveCtrl {
    ...
    @select(state => state.select_course.goodsId) productId;
    @select(state => state.live_goods_created_order_info.app) wxAppOrderInfo;
    @select(state => state.live_goods_created_order_info.scan) wxScanOrderInfo;
    orderType = {
        WX_APP: "app",
        WX_SCAN: "scan",
    };
    ...
    initFlags() {
        ...
        this.newFee=this.getStateService().params.fee;
    }
    ...
    createOrderWxPayApp() {
        ...
        this.liveService.createOrder(this.productId, this.payAppCode, "app").then(data=> {
            if (data.code == 200) {
                this.sendWxPayRequest();
                return;
            }
    ...
    createOrderWxPayScan() {
        ...
        this.liveService.createOrder(this.productId, this.payAppCode, "scan").then(data=> {
            if (data.code == 200) {
                this.showQrCode(); //显示二维码
                this.iscreateScanOrder = true;
                return;
     ...
     */
    gotoPayResult() {
        this.go("live_course_pay_result",{orderType:this.currentOrderType});
    }
     ...
     */
    gotoProtocol() {
        this.go("live_course_pay_protocol");
    }
}
