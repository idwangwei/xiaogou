...
@View('olympic_math_work_list', {
    url: '/olympic_math_work_list/:urlFrom',
    template: require('./olympic_math_work_list.html'),
    styles: require('./olympic_math_t_list.less'),
    inject:['$scope'
        , '$state'
        , '$stateParams'
...
class WorkListCtrl{
    ...
    timeCountCallBack(item) {
        let paperTime = Number(item.publishType == this.finalData.WORK_TYPE.FINAL_ACCESS ?
    ...
    getCountDownStartTime(item) {
        let paperTime = item.publishType == this.finalData.WORK_TYPE.FINAL_ACCESS ?
    ...
    initFlags() {
        ...
        this.isFinalAccess = this.$stateParams.urlFrom.indexOf(this.finalData.URL_FROM.FINAL_ACCESS) > -1; //当前作业列表为期末测评
        this.isFinalAccessStart = false;
    ...
    onBeforeEnterView() {
        let fromUrl = this.$stateParams.urlFrom || 'work_list';
    ...
    onBeforeLeaveView() {
        if (!this.isFinalAccess && this.stopTimer.timer) this.$interval.cancel(this.stopTimer.timer);
    ...
    loadCallback(loadMore, loadAllComplete) {
        ...
        if (this.getRootScope().isAfterSubmitPaperBackToWorkList && !this.isComeFromOlympicMath()) {//奥数提交作业后，不应该开通诊断
            ...
            if (this.isFirstSubmitWorkAfterUpdate) {
    ...
    getPaperStatusCallBack(param, col, $index,currentServerTime) {
        ...
        //期末测评作业
        if(col.paperInfo.publishType == this.finalData.WORK_TYPE.FINAL_ACCESS ){
            //测评时间已过
            ...
            if ((col.paperInfo.status == 1 && (systemTime > endTime || localNowTime > endTime)) //没点开过的试卷，超时不能再做
                ...
                this.commonService.showAlert('温馨提示', '<p style="text-align: center">答题时间已过，无法做题。</p>');
                return
            }
        }
    ...
    showReportFlag(listItem, index) {
        if(listItem.paperInfo.publishType == this.finalData.WORK_TYPE.FINAL_ACCESS){
            return true
        }
    ...
    getWorkClass(listItem) {
        ...
        className += listItem.paperInfo.status != 4 || (!this.showReportFlag(listItem)) ?
            ((listItem.paperInfo.publishType == this.finalData.WORK_TYPE.FINAL_ACCESS||listItem.paperInfo.publishType == this.finalData.WORK_TYPE.ORAL_WORK || listItem.paperInfo.publishType == this.finalData.WORK_TYPE.ORAL_WORK_KEYBOARD) ? ' ' : ' my-work-item-no-btn')
