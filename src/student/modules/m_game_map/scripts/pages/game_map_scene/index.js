...
@View('game_map_scene', {
    ...
    inject:[
        '$scope', '$state', '$rootScope', '$stateParams', '$ngRedux', 'themeConfig', 'gameMapInfoServer','gameGoodsPayServer'
    ]
})
...
class GameMapScene {
    ...
    @select(state => state.game_map_scene_list_processing) isDataFetching;
    ...
    configDataPipe() {
        this.dataPipe
            .when(() => !this.initCtrl && (this.normalClazz || this.selfStudyClazz))
            .then(() => {
                this.initCtrl = true;
    ...
    onAfterEnterView() {
        if(this.initCtrl && !this.isDataFetching &&(this.normalClazz || this.selfStudyClazz)){
            this.gameMapInfoServer.getSceneList();
        }
    }
