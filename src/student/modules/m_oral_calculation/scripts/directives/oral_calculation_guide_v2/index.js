...
export default function () {
    return {
        ...
        controller: ['$ionicTabsDelegate', '$scope', '$rootScope', '$ionicSlideBoxDelegate', '$timeout', '$ngRedux', 'commonService', 'finalData', ($ionicTabsDelegate, $scope, $rootScope, $ionicSlideBoxDelegate, $timeout, $ngRedux, commonService, finalData) => {
            ...
            $scope.isOralKeyboard = $scope.selectWork && $scope.selectWork.publishType === finalData.WORK_TYPE.ORAL_WORK_KEYBOARD;
            $scope.isAreaEvaluation = $scope.selectWork && $scope.selectWork.publishType === finalData.WORK_TYPE.AREA_EVALUATION;
            ...
            $rootScope.$watch("showOralCalculationGuideFlag", () => {
                if ($ionicTabsDelegate._instances.length)
                    $ionicTabsDelegate._instances[0].showBar(!$rootScope.showOralCalculationGuideFlag);
            });
        }],
