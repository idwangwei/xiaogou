...
@View('oral_calculation_select_question', {
    url: '/oral_calculation_select_question/:paperId/:paperInstanceId/:redoFlag/:urlFrom/:questionIndex',
...
class OralCalculationSelectQuestion {
    ...
    timeEndCallBack(retMsg) {
        ...
        this.commonService.showAlert(data.title, data.msg).then((res) => {
            if (res) {//点击确认
                retMsg ? this.go("home.work_list", 'back') : me.submitAnsAndPaper(me.submitCallback.bind(me), true);//改错后返回试卷列表
            }
        });
    }
    ...
    goUrlHandle() {
        ...
        if (this.getStateService().params.redoFlag === 'true')
            this.go("oral_calculation_work_detail", 'back');
        else
            this.go("home.oral_calculation_work_list", 'back');
    }
    ...
    submitCallback(msg) {
        ...
        this.$timeout(() => {  //不要删除这个定时器，不然做题时间会有问题
            ...
            if (this.getStateService().params.redoFlag === 'false') {
                ...
                this.go("home.oral_calculation_work_list", 'none');//改错后返回试卷列表
            }
