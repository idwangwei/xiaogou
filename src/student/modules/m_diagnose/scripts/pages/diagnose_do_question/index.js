 ...
 */
/**
 * Created by 邓小龙 on 2017/1/12.
 */
import {Inject, View, Directive, select} from '../../module';

@View('diagnose_do_question02', {
...
class DiagnoseDoQuestion02Ctrl{
    ...
    initData(){
        ...
        this.workStr = '还不够牢固';
        this.isFromFinalSprint=this.getStateService().params.backWorkReportUrl==='final_sprint_train';
    ...
    showErrorRecords(){
        ...
        if(this.backWorkReportUrl){
            this.go('diagnose_error_q_records','forward',{backWorkReportUrl:this.backWorkReportUrl});
        }else{
            this.go('diagnose_error_q_records','forward');
        }
    }
    ...
    back(){
        ...
        if(this.masterCode===4) {//已掌握情况点击按钮
            ...
            /*期末冲刺新增*/
            if(this.backWorkReportUrl=="final_sprint_train"){
        ...
        let callback=()=>{
            ...
            if(this.backWorkReportUrl){
                this.go('diagnose_knowledge02','back',{
                    'backWorkReportUrl':this.backWorkReportUrl,
                    isIncreaseScore:this.getRootScope().isIncreaseScore}
                );
            }else{
                this.go('diagnose_knowledge02','back',{isIncreaseScore:this.getRootScope().isIncreaseScore});
            }
    ...
    goToShowReport(){
        ...
        if(this.backWorkReportUrl){
            ...
            this.go('diagnose_report','forward',{urlFrom:'diagnose_do_question02',pointIndex:pointIndex,pointName:this.pointName})
        }
