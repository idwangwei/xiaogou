...
@View('micro_lecture_weixin_pay', {
    url: '/micro_lecture_weixin_pay',
...
class microLectureWeixinPayCtrl {
    $stateParams;
    microVipService;
    commonService;
    @select(state => state.select_micro_goods) microLectureInfo;
    @select(state => state.select_micro_goods.id) productId;
    @select(state => state.select_micro_goods.totalFee) totalFee;
    @select(state => state.select_micro_goods.title) payTitle;
    @select(state => state.select_micro_goods.num) productNum;
    @select(state => state.select_micro_goods.giftNum) productGiftNum;
    @select(state => state.micro_goods_created_order_info.app) wxAppOrderInfo;
    @select(state => state.micro_goods_created_order_info.scan) wxScanOrderInfo;
    orderType = this.microVipService.orderType;
    ...
    createOrderWxPayApp() {
        ...
        this.microVipService.createOrder(this.productId, this.payAppCode, "app").then(data=> {
            if (data.code == 200) {
                this.sendWxPayRequest();
                return;
            }
    ...
    createOrderWxPayScan() {
        ...
        this.microVipService.createOrder(this.productId, this.payAppCode, "scan").then(data=> {
            if (data.code == 200) {
                this.showQrCode(); //显示二维码
                this.iscreateScanOrder = true;
                return;
     ...
     */
    gotoPayResult() {
        this.go("micro_lecture_pay_result",{orderType:this.currentOrderType});
    }
     ...
     */
    gotoProtocol() {
        this.go("micro_lecture_pay_protocol");
    }
}

export default microLectureWeixinPayCtrl;