...
class WorkPubCtrl {
     ...
     */
    back() {
        let selectUnit = this.$ngRedux.getState().wr_selected_unit || {};
        let unitId = selectUnit.unitId || "";
        if (this.getStateService().params.from == 'eqc_redo_paper') {
            this.go('error_question_c', {urlFrom: 'work_pub'});
            return
        }
        if (this.getStateService().params.from == 'pub_work_preview') {
        ...
        this.go('work_chapter_paper', {from: 'work_pub', chapterId: unitId});
    };
