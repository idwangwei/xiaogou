...
import {PUB_WORK_DIFF_UNIT} from './../redux/action_types';
import {
...
@Service('areaEvaluationPubService')
@Inject("$q", "areaEvaluationInterface", "commonService", '$ngRedux')
class areaEvaluationPubService {
    $q;
    $ngRedux;
    commonService;
    ...
    getComposePaperQues(tempPaperSetParams, type) {
        ...
        return (dispatch, getState) => {
            ...
            dispatch({type: PUB_WORK_DIFF_UNIT.FETCH_AREA_EVALUATION_TEMP_PAPER_QUES_START});
            this.commonService.commonPost(this.areaEvaluationInterface.GET_TEMP_PAPER_QUES, param).then((data) => {
                if (data.code === 200) {
                    ...
                    dispatch({
                        type: PUB_WORK_DIFF_UNIT.FETCH_AREA_EVALUATION_TEMP_PAPER_QUES_SUCCESS,
                        payload: {
                            setParams: setParams,
                            quesList: quesList,
                    ...
                    dispatch({type: PUB_WORK_DIFF_UNIT.FETCH_AREA_EVALUATION_TEMP_PAPER_QUES_FAIL});
    ...
    composePaper(params) {
        return (dispatch, getState) => {
            let defer = this.$q.defer();
            dispatch({type: PUB_WORK_DIFF_UNIT.COMPOSE_AREA_EVALUATION_MULTI_UNIT_PAPER_START});
            this.commonService.commonPost(this.areaEvaluationInterface.GET_MULTI_UNITS_PAPER, params).then((data) => {
                if (data.code === 200) {
                    dispatch({
                        type: PUB_WORK_DIFF_UNIT.COMPOSE_AREA_EVALUATION_MULTI_UNIT_PAPER_SUCCESS,
                        payload: {paper: data.paper,type:data.type,questionCount:JSON.parse(params.qIds).length,paperName:params.paperName}
                    });
                    defer.resolve(true);
                } else {
                    dispatch({type: PUB_WORK_DIFF_UNIT.COMPOSE_AREA_EVALUATION_MULTI_UNIT_PAPER_FAIL});
                    defer.resolve(false, data.msg);
                }
    ...
    deleteSelectQues(tempPaperSetParams, questions) {
        return (dispatch, getState)=> {
            dispatch({
                type: PUB_WORK_DIFF_UNIT.CHANGE_AREA_EVALUATION_PAPER_SET_PARAMS, payload: {
    ...
    changePaperName(paperName) {
        return (dispatch, getState)=> {
            dispatch({
                type: PUB_WORK_DIFF_UNIT.CHANGE_AREA_EVALUATION_PAPER_NAME, payload: {
                    paperName: paperName
                }
            });
        }
...
export default areaEvaluationPubService;