 ...
 */
import {actionCreator,Inject} from 'ngDecoratorForStudent/ng-decorator';
import * as CREDITS_TASK from '../../redux/action_typs';
@Inject('$q', '$rootScope', 'commonService', 'serverInterface', '$ngRedux')
class CreditsInfoService {
    ...
    fetchTeaScoreList(param) {
        ...
        let post = this.commonService.commonPost(this.serverInterface.GET_TEACHER_SCORE_LIST, param, true);
    ...
    fetchTeaScoreDetail(param){
        return (dispatch, getState)=> {
            ...
            let post = this.commonService.commonPost(this.serverInterface.GET_TEACHER_SCORE_DETAIL, param, true);
            this.cancelRequestList.push(post.cancelDefer);
            let request = post.cancelDefer;
            post.requestPromise.then((data)=> {
                request = null;
