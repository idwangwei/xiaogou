...
module.exports = function(grunt) {
  grunt.registerMultiTask('scp', 'copy files to remote server.', function() {
    ...
    client.on('transfer', function(buf, up, total) {
      up = up + 1;
      // var rate = Math.floor(up / total * 100);
      // if (rate % 10 === 0) {
      //   grunt.log.write('transfer ' + rate + '% data\r');
      // }
      //var rate = up % 500
      if (up < total) {
        if ((Math.floor(up / 550)) === (up / 550)) {
          grunt.log.write('transfer ' + Math.floor(up / total * 100) + '% data\r');
        } else if (up === 1) {
          grunt.log.write('transfer 1% data\r');
        }
      } else {
        grunt.log.write('transfer ' + Math.floor(up / total * 100) + '% data\r');
      }
